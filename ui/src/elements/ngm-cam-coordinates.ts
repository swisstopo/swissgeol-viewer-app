...
export class NgmCamCoordinates extends LitElementI18n {
  ...
  updateCoordinates() {
    ...
    if (coords[0] < MIN_MAX_COORDINATE_VALUES[this.key].long.minValue || coords[0] > MIN_MAX_COORDINATE_VALUES[this.key].long.maxValue) {
      showSnackbarError(text);
      return;
    }
    if (coords[1] < MIN_MAX_COORDINATE_VALUES[this.key].lat.minValue || coords[1] > MIN_MAX_COORDINATE_VALUES[this.key].lat.maxValue) {
      showSnackbarError(text);
      return;
    }
