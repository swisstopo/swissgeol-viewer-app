...
export default class SwisstopoIdentify {
  ...
  async identify(position: Cartesian3, distance: number, layers: Array<string>, lang: string): Promise<IdentifyResult[]> {
    const carto = Cartographic.fromCartesian(position);
    const geom2056 = radiansToLv95([carto.longitude, carto.latitude]);
    const tolerance = getTolerance(distance);
    const results: IdentifyResult[][] = await Promise.all(layers.map(async layer => {
